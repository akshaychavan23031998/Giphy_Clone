  ...
  return (
    <div className={`flex my-3 gap-3 ${alignLeft ? "" : "justify-end"}
        ...
        <div className='flex min-w-80 rounded-full bg-gray-800'>
            {filters.map((f) => {
                return <span onClick={() => setFilter(f.value)} key={f.title} className={`${filter === f.value ? f.background : ""} font-semibold py-2 w-1/3 text-center rounded-full cursor-pointer`}>{f.title}</span>;
            })}
        </div>
    </div>
  )
